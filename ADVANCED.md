...
* **`bot <on|off|setup|status>`:** Управление Telegram-ботом. `setup` для привязки Bot Token и Chat ID.
* **`warp <on|off|status|mode>`:** Управление Cloudflare WARP. Режимы: `warp-only` (туннелирование через WARP) и `direct` (обычный режим).
* **`limit <имя> <down> <up>|clear`:** Установка лимитов скорости (например, `10mbit 5mbit`) или их очистка для клиента.
* **`server`:** Подробная системная статистика и состояние VPN-сервера.
...
sudo bash /root/awg/manage_amneziawg.sh bot setup

# Включить WARP (двойной VPN)
sudo bash /root/awg/manage_amneziawg.sh warp on

# Ограничить скорость клиента до 50 Мбит
sudo bash /root/awg/manage_amneziawg.sh limit my_phone 50mbit 50mbit

# Очистить лимиты
sudo bash /root/awg/manage_amneziawg.sh limit my_phone clear

# Посмотреть системную статистику
sudo bash /root/awg/manage_amneziawg.sh server
```

---

<a id="telegram-bot-adv"></a>
## 🤖 Telegram Бот

Скрипт включает в себя интерактивного Telegram-бота для удалённого управления сервером.

**Возможности:**
*   **Управление клиентами:** Добавление и удаление прямо из чата.
*   **Статистика:** Просмотр трафика по каждому клиенту.
*   **Мониторинг:** Кнопка "Сервер" показывает загрузку CPU, RAM, Disk и аптайм VPN.
*   **Безопасность:** Бот отвечает только на команды от администратора (по Chat ID).
...
3.  Запустите настройку: `sudo bash /root/awg/manage_amneziawg.sh bot setup`
4.  Включите бота: `sudo bash /root/awg/manage_amneziawg.sh bot on`

Сервис бота управляется через systemd (`awg-bot.service`).

---

<a id="warp-limits-adv"></a>
## 🌐 Cloudflare WARP и Лимиты скорости

### Cloudflare WARP
Интеграция позволяет перенаправлять трафик с вашего VPN-сервера в сеть Cloudflare. Это полезно для:
1.  **Скрытия IP вашего VPS** от конечных ресурсов.
2.  **Доступа к сервисам**, блокирующим IP дата-центров.
3.  **Обхода ограничений** самого хостинга.

**Команды:**
*   `warp setup`: Установка клиента `warp-cli` и регистрация.
*   `warp on/off`: Включение/выключение маршрутизации через WARP.
*   `warp mode`: Переключение между режимами (глобальный прокси или выборочно).

### Лимиты скорости (Traffic Control)
Используется системная утилита `tc` для ограничения полосы пропускания клиента.

**Важно:** Лимиты применяются "на лету" и сохраняются в конфигурации сервера. При перезагрузке интерфейса лимиты переприменяются автоматически.

**Пример:** `sudo bash /root/awg/manage_amneziawg.sh limit user1 100mbit 100mbit`
